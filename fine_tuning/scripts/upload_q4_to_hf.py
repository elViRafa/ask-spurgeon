...
def main():
    gguf_path = Path("fine_tuning/models/Spurgeon-8B-Q4_K_M.gguf")
    if not gguf_path.exists():
        print(f"[ERROR] Local GGUF file not found at: {gguf_path.absolute()}")
        print("Please ensure you have quantized the model to Q4_K_M GGUF format first!")
        sys.exit(1)

    print(f"[FOUND] Local Q4_K_M GGUF model at: {gguf_path.absolute()}")
    ...
    try:
        ...
        api.upload_file(
            ...
            repo_id=repo_id,
            repo_type="model",
            commit_message=f"Upload {gguf_path.name} (Q4_K_M quantized GGUF)"
        )
        print("\n[SUCCESS] Hugging Face Q4_K_M GGUF upload complete!")
