#!/usr/bin/env python3
"""
Ollama Synthetic Q&A Generation Script for Charles Spurgeon Fine-Tuning
Generates training data locally and for free using a local Ollama model.
"""
...
def call_ollama(prompt: str, model: str) -> str:
    """Call local Ollama API to generate response, enforcing JSON format."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"\n[OLLAMA ERROR] Status code {response.status_code}: {response.text}")
    except Exception as e:
        print(f"\n[OLLAMA EXCEPTION] Connection failed: {e}")
...
def main():
    parser = argparse.ArgumentParser(description="Generate high-quality Spurgeon Q&A dataset using Ollama.")
    parser.add_argument("--limit", type=int, default=1000, help="Number of Q&A pairs to generate")
    parser.add_argument("--output", type=str, default="fine_tuning/data/spurgeon_train_ollama.jsonl", help="Output JSONL file path")
    ...
    # Extract all chunks
    all_chunks = []
    # Sample training files randomly to vary contexts
    random.seed(100)
...
Write a brief response in Spurgeon's authentic voice and register, politely explaining that the provided context does not address this question. Do not refer to the prompt instructions. Keep it to 2-3 sentences."""
            ...
            qa_json_str = call_ollama(f"Sermon excerpt:\n{chunk}\n\nGenerate Q&A:", args.model)
            if not qa_json_str:
                continue
                
            try:
                pairs = json.loads(qa_json_str)
    ...
    pbar.close()
    print(f"\nSuccessfully generated {generated_count} high-quality Q&A examples using local Ollama model {args.model}.")
